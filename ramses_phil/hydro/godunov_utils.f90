...
subroutine cmpdt(uu,gg,dx,dt,ncell)
  ...
  write(*,*) 'Inside subroutine cmpdt, uu(ncell/2,1): ', uu(ncell/2.,1) !DWM
  do k = 1,ncell
     uu(k,1)=max(uu(k,1),smallr)
  end do
  write(*,*) 'subroutine cmpdt max between uu and smallr, uu(ncell/2,1): ', uu(ncell/2.,1) !DWM
  ! Velocity
  do idim = 1,ndim
     do k = 1, ncell
  ...
  write(*,*) 'about to zero uu for gravity-subroutine cmpdt, uu(ncell/2,1): ', uu(ncell/2.,1) !DWM
  do k = 1, ncell
     uu(k,1)=zero
  end do
  do idim = 1,ndim
