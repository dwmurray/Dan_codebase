subroutine hydro_flag(ilevel)
  ...
  use rt_parameters
#endif
  use jet_parameters, only : jet_refine
  implicit none
  integer::ilevel
  ! -------------------------------------------------------------------
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        if(jet_feedback .and. jet_refine)then
           call jet_feedback_refine(ind_cell,ind_grid,ind,ok,ngrid,ilevel)
        endif
...
!#####################################################################
!#####################################################################
subroutine jet_feedback_refine(ind_cell,ind_grid,ind,ok,ncell,ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  use poisson_commons
  use jet_parameters
  implicit none
  integer::ncell,ilevel,ind
  integer,dimension(1:nvector),intent(in)::ind_cell,ind_grid
  logical,dimension(1:nvector)::ok
  !-------------------------------------------------
  ! This routine sets flag1 to 1 if cell statisfies
  ! user-defined jet criterion for refinement.
  !-------------------------------------------------
  integer::i,indi,irad,indg,nx_loc
  real(dp),dimension(1:ndim) :: xc,skip_loc
  real(dp),dimension(1:ndim) :: xx,r_rel
  integer::ix,iy,iz
  real(dp)::scale,dx,dx_min
  integer::isink
  real(dp)::r_jet,rout
...
end subroutine jet_feedback_refine
