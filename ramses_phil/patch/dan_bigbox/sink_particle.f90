...
subroutine grow_sink(ilevel,on_creation)
  ...
  do isink=1,nsink
     ...
     else if (jet_feedback .and. sink) then
        delta_mass(isink)=delta_mass_all(isink) ! only the most recent accretion
     else
...
subroutine accrete_sink(ind_grid,ind_part,ind_grid_part,ng,np,ilevel,on_creation)
  ...
  use pm_commons
  use hydro_commons
  use poisson_commons, only : G_factor
  use jet_parameters
  implicit none
  integer::ng,np,ilevel
  ...
  real(dp),dimension(3)::nr_rel, z_axis ! normalized relative vector
  real(dp) :: jetValuePerVol, dtmdot, mdot_test = 1e-3*2e33/3.15e7, scale_m, vjet_test, sp_int_ener, ke, sp_v2
  real(dp) :: r_jet 
  ! Conversion factor from user units to cgs units
  call units(scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2)
  ...
  do ind=1,twotondim
     do j=1,np
        if(ok(j,ind))then
           ...
           !if (volume==0. .or. density==0.)print*,'something might be going wrong here...',weight,volume,density,ilevel,isink
...
subroutine read_sink_params()
  ...
  namelist/sink_params/n_sink,rho_sink,d_sink,accretion_scheme,nol_accretion,merging_timescale,&
       ...
       read_sink_on_restart
  namelist/jet_params/v_jet,theta0_jet,f_jet,rin_jet, rout_jet, jet_cell_supersample,test_jet_problem,test_rho,test_radius,test_spin,time_jet,jet_refine
...
112 rewind(1)
  ...
  if( jet_feedback) then
     rewind(1)
     read(1,NML=jet_params,END=113)
     cos_theta0_jet = cos(theta0_jet)
     if(myid==1)write(*,*)'Using Jet feedback'
     goto 114 
113  if(myid==1)write(*,*)'You did not set up &JET_PARAMS in the namelist file'
     if(myid==1)write(*,*)'Using default values '
114  rewind(1)
  end if
