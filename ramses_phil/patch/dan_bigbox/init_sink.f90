subroutine init_sink
  ...
  integer::ilun,nx_loc
  integer::nsinkold
  real(dp)::xx1,xx2,xx3,vv1,vv2,vv3,mm1,ll1,ll2,ll3,rr1
  ...
  allocate(rsink(1:nsinkmax))
  rsink = -1d0 ! set it to be uninitialized
  ...
  if(nrestart>0 .and. read_sink_on_restart)then
     ilun=4*ncpu+myid+10
     ...
     if(nsink>0)then
        ...
        read(ilun)xdp ! Read sink radius 
        rsink(1:nsink)=xdp
        deallocate(xdp)
...
!        read(ilun)ncloud_sink
  ...
  if (ic_sink)then
     ...
     eof=.false.                                                                                         
     do                                                                                                  
        read(10,*,end=102)mm1,xx1,xx2,xx3,vv1,vv2,vv3,ll1,ll2,ll3,rr1                                        
        nsink=nsink+1
        nindsink=nindsink+1
