     ...
     if(mod(nstep_coarse,ncontrol)==0.or.output_done)then
        if(myid==1)then
           ...
           if(cooling.or.pressure_fix)then
              ...
              write(*,777)nstep_coarse,mcons,econs*scale_ener,epot_tot*scale_ener,ekin_tot*scale_ener
           end if
#ifdef SOLVERmhd
  ...
  if(mod(nstep,ncontrol)==0)then
     if(myid==1.and.itest==0)then
        if(pic)then
           ...
           write(*,888)nstep,t,dt,aexp,&
                & real(100.0D0*dble(used_mem_tot)/dble(ngridmax+1))
           write(*,778)nstep_coarse,econs*scale_ener,epot_tot*scale_ener,ekin_tot*scale_ener,eint_tot*scale_ener !DWM
        endif
     end if
  end if
