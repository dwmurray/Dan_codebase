  ...
  do ind=1,twotondim
     do j=1,np
        if(ok(j,ind))then
           ...
           e=e/d
           do ivar=imetal,nvar
              ! Gathering up the values of scalars before
              ! density change...
              ! R. Sarmento - 5 Mar 2015
           ...
           if (on_creation)then
              ...
              uold(indp(j,ind),5)=e
              do ivar=imetal,nvar
                 ! Note that we have a new cell density, d
                 ! Using it to update the scalars... This
                 ! should work for iprist and iprimord ...
                 ! R. Sarmento - 5 March 2015
                 uold(indp(j,ind),ivar)=d*z(ivar)
!!$                 if ((ivar == iprist).AND.(aexp > 0.044160d0).AND.(aexp < 0.044161d0)) then
!!$                    print *,"sink: PGF", z(ivar)
!!$                    print *,"sink: Z", (uold(indp(j,ind),imetal)/d)
!!$                    print *,"sink: Z/(1-PGF)", (uold(indp(j,ind),imetal)/d)/(1.0d0- z(ivar))
!!$                    print *,"sink: rho", d,indp(j,ind)
!!$                 endif
              end do
           else
              ...
              do ivar=imetal,nvar
                 ! R. Sarmento
                 ! Assuming unew(,ivar) is the orig scalar, this computation will preserve
                 ! the old scalar value under the new density unew(indp(j,ind),1.
!!$                 if ((ivar == iprist).AND.(aexp > 0.044160d0).AND.(aexp < 0.044161d0)) then
!!$                    print *,"sink (before): PGF", (unew(indp(j,ind),iprist)/d)
!!$                 endif
                 unew(indp(j,ind),ivar)=unew(indp(j,ind),ivar)-uold(indp(j,ind),ivar)*acc_mass/(d*vol_loc)
!!$                 if ((ivar == iprist).AND.(aexp > 0.044160d0).AND.(aexp < 0.044161d0)) then
!!$                    print *,"sink (new): PGF", (unew(indp(j,ind),iprist)/d)
!!$                    print *,"sink (new): Z", (unew(indp(j,ind),imetal)/d)
!!$                    print *,"sink (new): Z/(1-PGF)", (unew(indp(j,ind),imetal)/d)/(1.0d0-(unew(indp(j,ind),iprist)/d)),indp(j,ind)
!!$                    print *,"sink (new): rho", d,indp(j,ind)
!!$                 endif
              end do
