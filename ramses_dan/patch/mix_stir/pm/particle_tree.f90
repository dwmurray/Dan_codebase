...
subroutine virtual_tree_fine(ilevel)
  ...
  ! Rick Sarmento - update to account for pristine fraction of 
  ! the star particle. As they are moved we need to move the
  ! pristine fraction data.
  ...
  if(star.or.sink) then
     ...
     ! Rick Sarmento - Add another 2 columns in the transfer array 
     ! for the pristine gas fraction
     ! Note that metal has to be TRUE for prist_gas_fraction 
     ! to be true. Again, should likely move it in the metal 'if'
     if(prist_gas_fraction) then
        particle_data_width=particle_data_width+2
     endif
  endif
...
subroutine fill_comm(ind_part,ind_com,ind_list,np,ilevel,icpu)
  ...
  if(star.or.sink)then
     ...
     ! Rick Sarmento
     ! Also need to copy the pfp (particle pristine fraction)
     ! into the reception struct when we move particles...
     ! Prob should move this in the 'metal' if, since it can only be true
     ! if metal is true... 
     if(prist_gas_fraction)then
        do i=1,np
           reception(icpu,ilevel)%up(ind_com(i),current_property+2)=pfp(ind_part(i))
           reception(icpu,ilevel)%up(ind_com(i),current_property+3)=z3p(ind_part(i))
        end do
     end if
  end if
  ...
  ! Rick S - Moving current_property along '2 more' if we are tracking the
  ! pristine gas fraction... This means we've used up 2 move slots in 'up'
  if(prist_gas_fraction) current_property = current_property+2
...
subroutine empty_comm(ind_com,np,ilevel,icpu)
  ...
  if(star.or.sink)then
     ...
     ! Rick Sarmento - If we're tracking the pristine fraction of the SPs, 
     ! restore them now...
     if(prist_gas_fraction)then
        do i=1,np
           pfp(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property+2)
           z3p(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property+3)
        end do
     end if
  end if
