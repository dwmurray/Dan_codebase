...
!##############################################################
!##############################################################
! Based on getnborfather, but returns an extra array that 
! identifies the ilevel of the nboring cell/grid
!##############################################################
!##############################################################
subroutine getnborfatherwithlevel(ind_cell,ind_father,nbor_level,ncell,ilevel)
  use amr_commons
  implicit none
  integer::ncell,ilevel
  integer,dimension(1:nvector)::ind_cell
  ...
  if(ilevel==1)then
     ...
     print *, "getnborfatherwithsize:",ilevel
     ! Get father cell
     do i=1,ncell
        ind_father(i,0)=ind_cell(i)
        nbor_level(i,0)= ilevel ! The cell is at ilevel
     end do

     ! Getting the indices of the nbors of ind_father(i,0)
     ! the central cell
     do i=1,ncell
        ix(i,3)=(ind_father(i,0)-1)/nxny
     end do
     ...
     do idim=1,ndim
        do i=1,ncell
           nbor_level(i,2*idim-1)= ilevel   ! Rick Sarmento - at the same level
           if(ix(i,idim)>0)then
              ind_father(i,2*idim-1)=ind_father(i,0)-iskip1(idim)
           else
              ind_father(i,2*idim-1)=ind_father(i,0)+iskip2(idim)
        ...
        do i=1,ncell
           nbor_level(i,2*idim)= ilevel   ! Rick Sarmento - at the same level
           if(ix(i,idim)<ibound(idim))then
              ind_father(i,2*idim)=ind_father(i,0)+iskip1(idim)
           else
              ind_father(i,2*idim)=ind_father(i,0)-iskip2(idim)
  ...
  else ! Where the real work is done: ilevel != 1

     ! Get father cell
     do i=1,ncell
        ind_father(i,0)=ind_cell(i)
        nbor_level(i,0)= ilevel
     end do
     ...
     ! Get neighboring father grids
     ! So igridn is two dim array where igridn(i,0) is 'this' grid
     call getnborgrids(ind_grid_father,igridn,ncell)

     ! Loop over position
     do ind=1,twotondim  ! twotondim = 8, going 1->8
        ...
        ! Get neighboring cells for selected cells
        ! icelln_ok are the returned indices of igrindn_ok's 
        ! nboring cells (igrindn_ok cell @ ind)
        if(iok>0)call getnborcells(igridn_ok,ind,icelln_ok,iok)
        ...
        do j=1,twondim
           ...
           do i=1,ncell
              if(pos(i)==ind)then
                 iok=iok+1
                 if(icelln_ok(iok,j)>0)then ! We have a cell nbor
                    ind_father(i,j)=icelln_ok(iok,j)
                    nbor_level(i,j)= ilevel   ! Rick Sarmento - at same level
                 else ! The nbor is an entire grid
                    ind_father(i,j)=nbor(ind_grid_father(i),j)
                    nbor_level(i,j)= ilevel-1 ! Rick Sarmento - at the father level
                 end if
              end if
           end do
        end do
...
end subroutine getnborfatherwithlevel
!##############################################################
!##############################################################
!##############################################################
!##############################################################
