subroutine read_hydro_params(nml_ok)
  ...
  ! Rick Sarmento
  ! Added a new boolean: prist_gas_fraction, turbulent_velocity
  ! If .true. the simulation will track the new scalar for
  ! each simulation cell
  ! Note that if prist_gas_fraction is TRUE, turbulent_velocity 
  ! will also be forced to be true
  ! 14 May 14 - RS added z_crit
  ! 24 Sep 14 - RS added h2_frac - the primordial H_2 fraction 
  ! h2_fraction is used in cooling. Default is 1e-6 via Reed, et al, 2005
  ! lw_photon - Lyman-Werner photons per stellar baryon. Greif & Bromm, 2006
  ! eta_sn3 - RS - the mass fraction returned to the ISM for PopIII SN
  namelist/physics_params/cooling,haardt_madau,metal,isothermal &
       ...
       & ,eta_sn, eta_sn3, yield,rbubble,f_ek,ndebris,f_w,mass_gmc,kappa_IR &
	   ...
	   & ,z_crit,h2_frac,lw_photon,prist_gas_fraction,turbulent_velocity
...
105 continue
  ...
  ! Rick Sarmento 
  ! Checks for consistent parameter sets
  !--------------------------------------------------
  ! Check for pristine_gas_fraction
  ! prist_gas_fraction TRUE means you also have to
  ! track metals, turbulent_velocity
  !--------------------------------------------------
  if(prist_gas_fraction.and.nvar<(ndim+5))then
     if(myid==1)write(*,*)'Error: pristine gas fraction needs nvar >= ndim+6'
     if(myid==1)write(*,*)'Modify NVAR in Makefile & recompile'
     metal=.true.
     turbulent_velocity=.true.
     nml_ok=.false.
  endif

  ! Ensure we have legacy behavior
  if ((eta_sn > 0.0) .and. (eta_sn3==0))then
     eta_sn3 = eta_sn
  endif

  ! Rick Sarmento 
  !--------------------------------------------------
  ! Check for turbulent_velocity
  !--------------------------------------------------
  if(metal.and.turbulent_velocity.and.nvar<(ndim+4))then
     if(myid==1)write(*,*)'Error: turbulent_velocity & metal needs nvar >= ndim+4'
     if(myid==1)write(*,*)'Modify NVAR in Makefile & recompile'
     nml_ok=.false.
  else if (turbulent_velocity.and.nvar<(ndim+3))then
     if(myid==1)write(*,*)'Error: turbulent_velocity & metal needs nvar >= ndim+3'
     if(myid==1)write(*,*)'Modify NVAR in Makefile & recompile'
     nml_ok=.false.
  endif
  ...
  ! Rick Sarmento
  ! Handle the indexing for the new scalars
  iprist=ichem
  if(neq_chem)iprist=ichem+1
  iturbvel=iprist
  if(prist_gas_fraction) iturbvel=iprist+1
  iprimordz=iturbvel
  if(prist_gas_fraction) iprimordz=iturbvel+1
