...
!=======================================================================
function mole_cool_prime(T2cell)
!=======================================================================
! Uses the derivative of our analytic H2 cooling curve
! at the temperature of interest to return a slope.

  use amr_parameters
  real(kind=8) :: nHcell, T2cell
  real(kind=8) :: mole_cool_prime
  real(kind=8) :: logT2, scaleFactor

  logT2 = log10(T2cell)
  ! Turn off molecular cooling if we're below about 250K
  if (logT2 < 2.4d1.or.h2_frac < 1.0d-10) then
     mole_cool_prime = 0.0
     return
  end if
  ! The high end of our model... 
  if (logT2 > 4.7d0) logT2 = 4.7d0

  mole_cool_prime = 11.314d0 - 2.0d0 * 1.19072d0 * logT2
  return
end function mole_cool_prime
!=======================================================================

!=======================================================================
function mole_cool(nHcell, T2cell)
!=======================================================================
! Compute a molecular cooling rate based on the current H2 fraction,
! number density & temperature in the cell
! We need cooling in erg s^-1 cm^3 to be compatible with units in 
! solve_cooling.
! R. Sarmento - 28 Sep 2014
!=======================================================================
  use amr_parameters
  real(kind=8) :: nHcell, T2cell
  real(kind=8) :: mole_cool
  real(kind=8) :: logT2, scaleFactor

  
  ! The following eqn is based on a fit to the cooling data in
  ! Martin & Mandy 96
  ! The data is derived from a log vs log plot so we need to take
  ! log of temperature to find the log of the cooling rate per nH atom
  ! The fit for nH = 1/cc ... This scales ~linearly up to nH = 1000
  ! -47.4999 + 11.314 x - 1.19072 x^2

  logT2 = log10(T2cell)
  
  ! The lower edge of our cooling table
  ! Turn off molecular cooling if we're below about 250K
  if (logT2 < 2.4d1.or.h2_frac < 1.0d-10)then
     mole_cool = 0.0
     return
  ...
  ! Truncate the range over which the curve fit applies
  ! If we're hotter than 50,000 K, use the value returned for 50k K
  ! The low end curve fit is fine - it behaves appropriately
  if (logT2 > 4.7d0)logT2 = 4.7d0
  !write (*,'(A, ES9.2, A, ES9.2)') "mole_cool nH = " ,nHcell," T2 = ",T2cell

  mole_cool =  -47.4999 + (11.314d0 * logT2) - (1.19072d0 * logT2**2d0)
  ! At this point mole_cool is log(cooling per H2 molecule) ... 
  mole_cool = 10d0**mole_cool ! Cooling per H2 molecule
  ! This is what I think we want... erg s^-1 cm^3 
  mole_cool = mole_cool * h2_frac / nhCell

  return

end function mole_cool
!=======================================================================

subroutine solve_cooling(nH,T2,zsolar,boost,dt,deltaT2,ncell)
!=======================================================================
! R. Sarmento 
! This routine gets its parameters in physical units. First 6 entries are
! arrays.
! So nH is in H/cc (verified)
! -- note that in cooling_fine nH is first used in user units to 'unscale'
! -- the other quantities. Then it is multiplied by scale_nH
! T2 is cell temp in T/mu in K but with t2min subtracted out. This is 
! a thermal temperature. See cooling_fine:
! zsolar is z in solar units
! boost = 1 in our case (note using ATON)
! dt = time step in seconds
! deltaT2 is the result passed to caller
  implicit none  
  integer::ncell
  real(kind=8)::dt
  ...
  real(kind=8)::lambda,lambda_prime,logT2max,mole_cooling, no_mole, mole_cooling_prime, lp_no_mole
  ...
  ! R. Sarmento - **** all table values are in log10 ****  
  ! Initializations
  logT2max=log10(T2_max_fix)
  ! 1/dlog_nH is the log increment for between each entry in the table.
  dlog_nH=dble(table%n1-1)/(table%nH(table%n1)-table%nH(1))
  ! table%n1 = 161, table%nH(1) = -10.0, table%nH(161) = 6, so dlog_nH = 10
  ! Conversely, each table entry is seperated by 0.1 (in log space)
  dlog_T2=dble(table%n2-1)/(table%T2(table%n2)-table%T2(1))
  ! table%n2 = 101, table%T2(101) = 9, table%T2(1) = -2, so dlog_T2 = 9.091
  ! So each table entry is seperated by 0.11 (in log space)
  h=1d0/dlog_T2
  h2=h*h
  h3=h2*h
  precoeff=2d0*X/(3d0*kB)
  do i=1,ncell
     zzz(i)=zsolar(i) ! Metallicity in solar units
     ! facH ensures nH is in the table%nH range -- where 
     ! table%nH(1) is log10(min_nH) & table%nH(table%n1) is log10(max_nH)
     ! So facH is a proxy for nH but guaranteed to be in the range above. 
     facH(i)=MIN(MAX(log10(nH(i)/boost(i)),table%nH(1)),table%nH(table%n1))
     ! i_nH(i) is the index into the table%nH closest to value facH
     ! This will be the index at or below facH since int() truncates
     i_nH(i)=MIN(MAX(int((facH(i)-table%nH(1))*dlog_nH)+1,1),table%n1-1)
     w1H(i)=(table%nH(i_nH(i)+1)-facH(i))*dlog_nH ! (next bigger table entry - real nH) * dlog_h
     w2H(i)=(facH(i)-table%nH(i_nH(i)  ))*dlog_nH ! (real nH - lower or equal table entry) * dlog_h
     ! So w#H's are WEIGHTING factors... They identify how close you are to the next lower and upper table
     ! entires on a scale of 0->1
     tau(i)=T2(i)
     tau_ini(i)=T2(i)
  ...
  do while(n>0)
     ...
     iter=iter+1
     if (iter > 1000) then ! Increased max iter to 1000 (from 500)
     ...
     n_active=0
     do i=1,n
        facT=log10(tau(ind(i))) ! Log of cell temp

        if(facT.le.logT2max)then
           ! Find index of temp 'facT' in the table (limited range)
           i_T2=MIN(MAX(int((facT-table%T2(1))*dlog_T2)+1,1),table%n2-1)
           yy=facT-table%T2(i_T2) ! Diff between act temp and table value (log)
           yy2=yy*yy
           yy3=yy2*yy

           ! Cooling
           ! R. Sarmento -
           ! Cooling table indexed by log nH AND temperature - interpolate
           ! Table values (cool, heat, etc) are all log values... 
           ! ***
           ! fa is lower cooling value time scale factor (how far above that entry we are... (0->1)
           ! plus the cooling value of the next higher density time scale factor how close we are to it
           fa=table%cool(i_nH(ind(i)),i_T2  )*w1H(ind(i))+table%cool(i_nH(ind(i))+1,i_T2  )*w2H(ind(i))
           ! Interpolate based on temperature AND density next.
           fb=table%cool(i_nH(ind(i)),i_T2+1)*w1H(ind(i))+table%cool(i_nH(ind(i))+1,i_T2+1)*w2H(ind(i))
           ! Same for the derivative table
           ...
           ! Haven't figured this out, but it's some kind of spline ... 
           alpha=fprimea
           beta=3d0*(fb-fa)/h2-(2d0*fprimea+fprimeb)/h ! h2 is (1d0/dlog_T2)^2
           gamma=(fprimea+fprimeb)/h2-2d0*(fb-fa)/h3
           ! Back to real units... erg/s cm**3
           ...
           ! R. Sarmento - 25 Sep 2014
           ! Add in molecular cooling
           ! mole_cool already returns 10**mole_cool... 
           ! don't need to '10**' here.

           ! ES attempt Oct 22
           ! fa = mole_cool(nH(i),T2(i))
           ! fb = mole_cool(nH(i),T2(i)*10d0**(1d0/dlog_T2))
           ! fprimea = mole_cool_prime(nH(i),T2(i))
           ! fprimeb = mole_cool_prime(nH(i),T2(i)*10d0**(1d0/dlog_T2))
           ! alpha=fprimea
           ! beta=3d0*(fb-fa)/h2-(2d0*fprimea+fprimeb)/h
           ! gamma=(fprimea+fprimeb)/h2-2d0*(fb-fa)/h3
           ! mole_cooling =10d0**(fa+alpha*yy+beta*yy2+gamma*yy3)               
           ! mole_prime=metal/tau(ind(i))*(alpha+2d0*beta*yy+3d0*gamma*yy2)

           ! R. Sarmento - since we don't do table look-ups, and have
           ! an analytic function for mole_cooling, we can implement
           ! a direct derivative of that function to find mole_prime
           mole_cooling = mole_cool(nH(ind(i)),tau(ind(i)))
           mole_cooling_prime = mole_cool_prime(tau(ind(i)))
           !mole_cooling = 0.0
           !mole_cooling_prime = 0.0

           ! Total cooling
           ! Note that they are dividing lambda by nH
           lambda=cool+zzz(ind(i))*metal-heat+(cool_com-heat_com)/nH(ind(i))

           ! R. Sarmento - Add molecular cooling, keep track of pre-molecular cooling 
           ! so we can compare... mole_cooling has been multiplied by h2_fraction
           no_mole = lambda
           lambda = lambda + mole_cooling

           lp_no_mole = cool_prime+zzz(ind(i))*metal_prime-heat_prime+(cool_com_prime-heat_com_prime)/nH(ind(i))
           lambda_prime= lp_no_mole + mole_cooling_prime

           ! Print out info when molecular cooling is an order-of-mag > standard cooling
!           if (mole_cooling * 1.0d1 > no_mole .and. no_mole > 0.0) then
!              write(*,'(A, I3, 14(A, ES9.2))') "Cell: ", i, " T2: ", T2(i), " nH: ", nH(i), &
!                   "Tot cool NO  molecular: ",no_mole, " Mole cooling: ",mole_cooling, " Tot cool w/ molecular: ",lambda, &
!                   "lambda prime NO mole = ",lp_no_mole, " Mole prime ", mole_cooling_prime, " lambda prime = ",lambda_prime, &
!                   "dT2 mole cool ", 10d0**(1d0/dlog_T2), " tau= ",tau(ind(i)), &
!                   " metal ", metal, " metal_prime ", metal_prime, " term ", (alpha+2d0*beta*yy+3d0*gamma*yy2), " tau ", tau(ind(i))
!              write (*,*) " ******* "
!           end if
        else ! Off the table -- temp too high

           ! Bremstrahlung cooling... Units erg s^-1 cm^3 ... (from Romain's email)
           lambda=1.42*1d-27*sqrt(tau(ind(i)))*1.1
           lambda_prime=lambda/2./tau(ind(i))

           !mole_cooling = mole_cool(nH(i),T2(i))
           !write(*,'(A, I3, 5(A, ES9.2))') "Bremstrahlung cooling, cell ", i, " T2= ", T2(i), &
           !     "  Mole cooling= ",mole_cooling, " Total cooling with molecular= ",lambda, &
           !     "  lambda prime = ",lambda_prime, " tau= ",tau(ind(i))
        endif
...
!=======================================================================
  ...
  real(kind=8)::nH_factor, T2_factor ! R. Sarmento - optimization
  integer::nbin_n,nbin_T
...
#endif  
  nH_factor = (log10(nH_max)-log10(nH_min))
  do i_n=1,nbin_n
     ! R. Sarmento - The following generates nbin_n equally spaced numbers between
     ! log10(nH_min) and log10(nH_max)
     tmp=log10(nH_min)+(dble(i_n)-1d0)/(dble(nbin_n)-1d0)*nH_factor
     table%nH(i_n)=tmp
  end do
  T2_factor = (log10(T2_max)-log10(T2_min))
  do i_T=1,nbin_T
     ! R. Sarmento - The following generates nbin_T equally spaced numbers between
     ! log10(T2_min) and log10(T2_max)
     tmp=log10(T2_min)+(dble(i_T)-1d0)/(dble(nbin_T)-1d0)*T2_factor
     table%T2(i_T)=tmp
  end do
...
!=======================================================================
! R. Sarmento
! Called with a density, i_n... Builds the table entries for that
! density for a range of temperatures: table%T2(1->nbin_T)
  implicit none
  integer :: i_n
  real(kind=8),dimension(1:3)::t_rad_spec,h_rad_spec
  real(kind=8) :: aexp
  ...
  do i_T = 1,nbin_T
     T2=10d0**table%T2(i_T) ! So T2 here is act temp, not log...
     ! Compute cooling, heating and mean molecular weight
     call cmp_cooling(T2,nH,t_rad_spec,h_rad_spec,cool_tot,heat_tot,cool_com,heat_com,mu,aexp,n_spec)
     table%cool(i_n,i_T)=log10(cool_tot) ! So all data in the tables is log10... R. Sarmento
...
!=======================================================================
! The routine populates the cooling table.
  implicit none
