  ...
  do ind=1,twotondim
     ...
     do i=1,active(ilevel)%ngrid
        ...
        write(*,*)'Stir Source. rho: ', d, 'u,v,w:', u,v,w &
             & 'accel ax,ay,az', ax,ay,az,'unew(,5): ', unew(ind_cell, 5) !DWM
...
subroutine godfine1(ind_grid,ncache,ilevel)
  ...
  use hydro_commons
  use poisson_commons
  use stir_parameters ! DWM 05/2019 Stirring
  implicit none
  ...
  do i1=i1min,i1max
     ...
     if(nbuffer>0)then
        ...
        do j=0,twondim
           do ivar=1,nvar
              do i=1,nbuffer
                 u1(i,j,ivar)=uold(ibuffer_father(i,j),ivar) ! Is this an error?
              end do
           end do
        end do
        ! R. Sarmento - I need to set the interpol type to 
        ! direct before doing this interpolation. 
!        print *,"interpolate hydro variables from parent cells"
        orig_type = interpol_type ! FORCE type 0
        interpol_type = 0 ! FORCE type 0
        call interpol_hydro(u1,u2,nbuffer)
        interpol_type = orig_type ! FORCE type 0
     endif
  ...
  do idim=1,ndim
     ...
     do i2=i2min,i2max
        ...
        do ivar=1,nvar
           if (ivar == iturbvel) cycle ! DON'T mix iturbvel - RS
           do i=1,ncache
  ...
  do idim=1,ndim
     ...
     nb_noneigh=0
     do i=1,ncache
        ! I think this is asking if there are cells at the same level as
        ! ind_grid(i) ... if not, get the father cell for the nbor
        ! and store in ind_buffer. 
        ! R. Sarmento
     ...
     ! Conservative update of new state variables
     nvarskip=0 ! DWM 05/2019
     if(stir) nvarskip=stir_nvar
     do ivar=1,nvar-nvarskip ! DWM 05/2019:  do ivar=1,nvar
        ! Loop over boundary cells
        if (ivar == iturbvel) cycle ! DON'T mix iturbvel - RS
