subroutine init_part
  ...
  if(star.or.sink)then
     ...
     if(prist_gas_fraction) then
        ! Adding new array to track pristine
        ! fraction for star particles.
        allocate(pfp(npartmax))
        pfp=1.0D0
        ! Adding z3p to track the Z of the star particle
        ! due only to POP III star nucleosynthesis
        ! Directly inherited from the gas... uold(*,iprimordz)
        allocate(z3p(npartmax))
        z3p=0.0D0  ! Stars start with Z due to POP III = 0
     end if
  end if
  ...
  if(nrestart>0)then
     ...
     if(star.or.sink)then
        ...
        if(prist_gas_fraction)then
           ! Read the particles' pristine fraction - R. Sarmento
           read(ilun)xdp
           pfp(1:npart2)=xdp
           ! Read the particles' primordial Z fraction - R. Sarmento
           read(ilun)xdp
           z3p(1:npart2)=xdp
        end if
        deallocate(xdp)
  ...
  else     ! Begin: FRESH start to a run
     ...
     case ('grafic')
        ...
        if(star.or.sink)then
           do ipart=1,npart
              ...
              if(metal)then
                 !zp(ipart)=0d0
                 zp(ipart)=z_ave ! R. Sarmento bug fix... 
              end if
           end do
        end if
