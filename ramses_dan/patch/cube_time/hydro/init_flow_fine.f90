...
!!$#endif
           ...
           if(ncache>0)then
              ...
              if(cosmo.and.ivar==imetal.and.metal)then
                 init_array=z_ave*0.02 ! from solar units
              endif
              ! Rick Sarmento - Nov 2013
              ! Add initialization for ipristine, the pristine fraction, here
              if(cosmo.and.(ivar==iprist).and.prist_gas_fraction)then
                 init_array=1.0d0 ! The gas pristine fraction starts at 1.0
              endif
              ! Don't need to do this since we want the primordial Z
              ! initialized to 0.0... R. Sarmento.
              ! Leaving it here for reference... 
              !if(cosmo.and.ivar==iprimordz.and.prist_gas_fraction)then
              !   init_array=0.0 ! The Z for gas generated by POP III stars stars @ 0.0
              !endif
              ...
              !if ivar==iturbvel we want to init_array=0.0: it is init'd above 
           endif
        endif
...
!!$              if (ivar == iprist) then
!!$                 print *,"init_flow init iprist bef scaling = ", uold(icell,ivar)
!!$              end if
           end do
        end do
        ! End loop over cells
     ...
     do igrid=1,ncache,nvector
        ...
        do ind=1,twotondim
           ...
           ! Rick Sarmento - Note that this will also init
           ! the new iprist ivar
           do ivar=ndim+3,nvar !DWM N.B.
              do i=1,ngrid
                 rr=uold(ind_cell(i),1)
                 ! Rick Sarmento- uold(,1) is density ... 
                 ! These scalars are normalize by the density -> uold(*,1)
                 ! So the pristine gas fraction is too!
                 uold(ind_cell(i),ivar)=rr*uold(ind_cell(i),ivar)
!!$                 if (ivar == iprist) then
!!$                    print *,"init_flow iprist scaled by rho: ", uold(ind_cell(i),ivar)
!!$                    print *,"init_flow rho:", uold(ind_cell(i),1)
!!$                 end if
              end do
           enddo
#endif
     ...
     do igrid=1,ncache,nvector
        ...
        do ind=1,twotondim
           ...
           do ivar=1,nvar
              do i=1,ngrid
                 ! Rick Sarmento
                 ! uu is set in condinit, where it is parameter u
...
!################################################################
! Rick Sarmento
! This routine is called by condinit::condinit
! condinit uses the "q" parameter setup by this function to 
! initialize the cell parameters
! It also loops over the init conds for the regions... and updates
! the q's ... copying in 'old' values (??)
...
#if NVAR > NDIM + 2
  do ivar=ndim+3,nvar !DWM N.B. R.S. sets all passives to 0.0
     q(1:nn,ivar)=0.0d0
     ...
     if (ivar==iprist)then
        ...
        if(verbose)print *,"init_flow_fine::region_condinit: iprist Init q(1:nn,iprist)=1.0", ivar, iprist
        q(1:nn,ivar)=1.0d0 ! Want to set this to 1... 
     endif
     if (ivar==iturbvel)then
        if(verbose)print *,"init_flow_fine::region_condinit: iturbvel Init q(1:nn,i)=0.0", ivar, iturbvel
     endif
     if (ivar==iprimordz)then
        if(verbose)print *,"init_flow_fine::region_condinit: iprimordz Init q(1:nn,i)=0.0", ivar, iprimordz
     endif
  end do
#endif
  ...
  do k=1,nregion
     ...
     if(region_type(k) .eq. 'square')then
        ...
        do i=1,nn
           ...
           ! Rick Sarmento
           ! I think, if r < 1.0 we have sub-cells -- hence we
           ! already have some values to copy in.
           if(r<1.0)then
              q(i,1)=d_region(k)
              q(i,2)=u_region(k)
#if NDIM>1
