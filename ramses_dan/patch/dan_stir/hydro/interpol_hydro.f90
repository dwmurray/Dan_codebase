...
subroutine interpol_hydro(u1,u2,nn)
  ...
  do ivar=1,nvar
     ...
     do ind=1,twotondim
        ...
        do idim=1,ndim
           do i=1,nn
              ! if interpol_type == 0, then w = 0 so we get
              ! str8 injection
              !! DEBUG **** - RS
!!$              if (ivar == iturbvel .and. (w(i,idim)*xc(ind,idim) /= 0.0)) then
!!$                 print *,"interpol_hydro: before v_t (km/s) ", u2(i,ind,ivar) * scale_v/1.0d5
!!$                 print *,"interpol_hydro: after v_t (km/s), w*xc ", (u2(i,ind,ivar)+w(i,idim)*xc(ind,idim)) * scale_v/1.0d5, w(i,idim)*xc(ind,idim)
!!$              end if
              !! DEBUG END 
