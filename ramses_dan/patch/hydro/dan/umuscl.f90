...
subroutine unsplit(uin,gravin,flux,tmp,dx,dy,dz,dt,ngrid)
  ...
  do k=klo,khi
     do ivar=1,nvar
        do l=1,ngrid ! Fix for performance... move do inside the if and the else...  RS
           if (ivar == iturbvel) then 
              flux(l,i,j,k,ivar,1) = 0.0 ! DON'T MIX iturb
           else 
  ...
  do k=klo,khi
     do ivar=1,nvar
        do l=1,ngrid
           if (ivar == iturbvel) then 
              flux(l,i,j,k,ivar,2) = 0.0 ! DON'T MIX iturb - RS
           else 
  ...
  do k=kf1,kf2
     do ivar=1,nvar
        do l=1,ngrid
           if (ivar == iturbvel) then 
              flux(l,i,j,k,ivar,3) = 0.0 ! DON'T MIX iturb - RS
           else 
...
!              qm(l,i,j,k,ir,3) = max(smallr, qm(l,i,j,k,ir,3))
  ...
  ! Passive scalars
  do n = ndim+nener+3, nvar
     if ( n == iturbvel ) cycle ! We don't mix iturbvel: don't waste time computing - RS
     do k = klo, khi
        do j = jlo, jhi
           do i = ilo, ihi
              do l = 1, ngrid
  ...
  do k = klo, khi
     do j = jlo, jhi
        do i = ilo, ihi
           ...
           do n = ndim+3, nvar
              if ( n == iturbvel ) cycle ! Don't mix the turb vel. - RS
              do l = 1, ngrid
