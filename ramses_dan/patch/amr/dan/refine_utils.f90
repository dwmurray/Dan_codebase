...
#else
        ! R. Sarmento - ensure straight 
        ! injection to children
        orig_type = interpol_type
        interpol_type = 0
        call interpol_hydro(u1,u2,nn)
        interpol_type = orig_type
#endif
        ...
        do j=1,twotondim
           ...
           do ivar=1,nvar+3
              ...
              do ivar=1,nvar
                 ...
                 do i=1,nn
                    ! R. Sarmento DEBUG
!!$                    if ((ivar == iprist) .AND. (uold(iskip+ind_grid_son(i),iprist)/uold(iskip+ind_grid_son(i),1) > 1.001d0)) then
!!$                       print *,"refine_utils, pre-interpol iprist ",uold(iskip+ind_grid_son(i),iprist)/uold(iskip+ind_grid_son(i),1)
!!$                    end if
                    uold(iskip+ind_grid_son(i),ivar)=u2(i,j,ivar)
!!$                    if ((ivar == iprist) .AND. (u2(i,j,iprist)/u2(i,j,1) > 1.001d0)) then
!!$                       print *,"refine_utils, post-interpol iprist ",u2(i,j,ivar)/u2(i,j,1)
!!$                    end if
                 end do
#ifdef SOLVERmhd
              end do
