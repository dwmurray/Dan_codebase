recursive subroutine amr_step(ilevel,icount)
  ...
  if(ilevel==levelmin)then
     ...
     !----------------------------------------------------
     ! Rick Sarmento
     ! Update the pristine gas fraction - mixing reduces
     ! the pristine gas fraction in each cell (assuming
     ! it isn't already effectively 0)...
     ! This routine is only called at the course level
     if(prist_gas_fraction)call pristine_update

  endif
  ! Call pristine update at every level ... need to update
  ! turb velocity at all levels!
