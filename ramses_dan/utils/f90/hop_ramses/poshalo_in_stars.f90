program poshalo
...
#else
  print *,"poshaloStars: Number of particles: ",npart
  ...
  do icpu=1,ncpu
     ...
     do i=1,npart2
        if(nstar>0) then
           ! if(age(i)/=0.or.id(i)<1) cycle ! Skipping star particle with age != 0.0
           if(age(i) == 0) cycle ! Skipping DM particle with age == 0.0 - RS
        end if
        npart=npart+1
