...
int ReadRamses(KD kd,char *inputfile)
{
  int dummy,npart,npart2,i,j,icpu,ncpu,ndim,ncurr,nstar_tot,nsink,nstar,otherTotStars;
  int localseed[4];
  ...
  if(nstar>0){
    ...
    printf("Removing DM from particle list. \n");

    ncurr=0;otherTotStars=0;
    for(icpu=1;icpu<=ncpu;icpu++)
      {
	...
	//Reading age
	fread(&dummy, sizeof(dummy), 1, fp);
	fread(&temp_dbl[0],sizeof(double),npart2,fp); // So particle age is in temp_dbl
	fread(&dummy, sizeof(dummy), 1, fp);
	
	for(j=0;j<=npart2-1;j++)
	  {
	    /* if(temp_dbl[j]==0. && temp_id[j]>0)ncurr=ncurr+1; // RS- Using age to count number of DM particles */
	    // age == 0 then we have a dm particle... 
	    if (temp_dbl[j] < 0.0) ncurr=ncurr+1; // RS- Using age < 0 --> to count number of STAR particles
	  }
    ...
    npart=ncurr; // Number of STAR particle across all CPUs - RS

  }

  printf("Actual number of STAR particles = %d \n",npart);
  printf("nstar_tot from first read: %d\n",otherTotStars);
  ...
  for(icpu=1;icpu<=ncpu;icpu++)
	...
	//Reading age
	fread(&dummy, sizeof(dummy), 1, fp);
	fread(&a_dbl[0],sizeof(double),npart2,fp); // a_dbl is particle age. - RS
	fread(&dummy, sizeof(dummy), 1, fp);

	// Just copy out the star particle locations! - RS
	for(j=0;j<=npart2-1;j++)
	  {
	    //	    printf("Star particle id %d, age = %f\n",temp_id[j],a_dbl[j]);
	    //	    if(a_dbl[j]==0.&&temp_id[j]>0){ // RS- So DM must have id > 0 and age == 0 ... 
	    if(a_dbl[j] < 0){ // RS- So DM must have id > 0 and age == 0 ... Star Particles have age < 0
