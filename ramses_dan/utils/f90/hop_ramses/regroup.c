...
/* I think this will work even if saddledensthresh<densthresh */
{
    ...
    int *densestboundgroup, changes, retVal;
    char line[80], tempfilename[] = "/tmp/tempFile_XXXXXX"; 
    FILE *fp;
    FILE *boundfp;
    
    printf("densthresh= %f\n",densthresh);
    printf("Template Temp file name: %s\n",tempfilename); 
    if (retVal=mkstemp(tempfilename) == -1) {
	printf("UNABLE to create temp file name in merge_groups_boundaries");
	printf("mkstemp return value = %d",retVal);
	exit(-1);
      }
    printf("merge_groups_boundaries tmpfilename: %s\n",tempfilename);
...
/* If fname==NULL, write to stdout */
{
    ...
    printf("writetags: num parts %d\n",s->numpart);
    fwrite(&(s->numpart),4,1,f);
    printf("writetags: num groups %d\n",gl->ngroups);
...
/* Use a format readable for FORTRAN unformatted read commands */
{
    ...
    dummy = 8; fwrite(&dummy,4,1,f); // dummy (8) is a flag in a fortran binary file
    printf("writetags: num parts %d\n",s->numpart);
    fwrite(&(s->numpart),4,1,f);
    printf("writetags: num groups %d\n",gl->ngroups);
    fwrite(&(gl->ngroups),4,1,f);
...
//fwrite((UINT64 *) rbuffer12,8,((nacq/2) * 4 * channels_number *acq_length) ,fd_raw); 
//fwrite((UINT64 *) rbuffer12,8,((nacq/2) * 4 * channels_number *acq_length) ,fd_raw); 
//fclose(fd_raw);
